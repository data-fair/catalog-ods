...
const list = async (config: ListContext<ODSConfig, typeof capabilities>): Promise<{ count: number; results: (Folder | Resource)[]; path: Folder[] }> => {
  ...
  let res
  try {
    res = (await axios.get(`${config.catalogConfig.url}/catalog/datasets?select=exclude(features),exclude(attachments),exclude(alternative_exports),exclude(fields)`, { params: odsParams })).data
  } catch (e) {
    console.error(`Error fetching datasets from ODS ${e}`)
    throw new Error('Error fetching datasets from ODS')
  }
...
const getResource = async (catalogConfig: ODSConfig, datasetId: string): Promise<Resource> => {
  let dataset
  try {
    dataset = (await axios.get(`${catalogConfig.url}/catalog/datasets/${datasetId}?select=exclude(features),exclude(attachments),exclude(alternative_exports),exclude(fields)`)).data
  } catch (e) {
    console.error(`Error fetching datasets from ODS ${e}`)
    throw new Error('Error fetching datasets from ODS')
  }
...
/**
 * Returrns the rows of a dataset that match the given constraints
 * @param catalogConfig the ODS configuration [ex: { url: 'https://example.com/api/explore/v2.1' }]
 ...
 */
const getRowsWithAValue = async (catalogConfig: ODSConfig, datasetId: string, constraints: { field: string; valeurs: string[] }[]): Promise<any[]> => {
  ...
  if (constraints && constraints[0] && constraints[0].field) {
    ...
    odsParams.where = constraints.map((cons: { valeurs: any[]; field: any; }) => {
      return cons.valeurs.map((val: any) => {
        return `${cons.field} = '${val}'`
      }).join(' or ')
    }).join(' and ')
  }

  try {
    const dataset = (await axios.get(`${catalogConfig.url}/catalog/datasets/${datasetId}/exports/csv`, { params: odsParams })).data
    return dataset
  } catch (error) {
    console.error(`Error fetching dataset values ${error}`)
    throw new Error('Error fetching dataset values')
  }
}
