 ...
 */
const getRowsWithAValue = async (catalogConfig: ODSConfig, datasetId: string, constraints: { field: { name: string; type: string }; valeurs: { name: string }[] }[]): Promise<any[]> => {
  var odsParams = {
    select: '*',
    where: ''
  }
    ...
    odsParams.where = constraints.map((cons: { valeurs: any[]; field: { name: string; type: string }; }) => {
      return cons.valeurs.map((val: any) => {
        switch (cons.field.type) {
          case "text":
            return `${cons.field.name} = \"${val.name}\"`;
          case "int":
          case "double":
            return `${cons.field.name} = ${val.name}`;
          case "date":
          case "datetime":
            return `${cons.field.name} = date'${val.name}'`;
          default:
            return `${cons.field.name} is ${val.name}`;
        }
      }).join(' or ')
    }).join(' and ')
  }
